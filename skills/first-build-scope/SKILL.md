...
description: Helps a pre-product founder (no artifact, no team, no specific technology yet) think through what to build at the idea stage. This skill owns the "how do I turn this idea into a first build" moment — the feasibility-shaping phase — and chains naturally after a brainstorming or intent-exploration session that has already clarified WHAT the product should be and WHO it's for. Use this skill PROACTIVELY whenever the user describes a product idea they want to build but have not yet started, asks "how should I build X" at the idea stage, mentions wanting to build an app/SaaS/tool/platform without naming the stack, asks for help thinking through a build, surfaces an idea that needs feasibility shaping before any specific technical decisions are made, OR has just finished clarifying a product idea and is ready to figure out how to actually build the first version. Common trigger phrases include "I want to build an app that does X", "I'm planning a SaaS for Y", "I have an idea for a tool that helps Z", "how would I build a product like X", "what would it take to build X", "I'm thinking about starting a product that does X", "what stack should I use for a new X", "I want to start building X", and the post-brainstorm handoff phrases "now how do I actually build this", "we've clarified the idea, what's the build", "what would it take to ship a first version", "ok how do I make this real". Also triggers when the user describes the product idea in user-need language ("I want users to be able to X" with no existing system context), or when they explicitly say they have no engineers, no codebase, just an idea. Especially important to trigger when the user is non-technical (vibe coder, PM, or first-time founder) and needs to see the option-space before being asked to commit. Produces a "build map" — a learning document that lays out 3–6 viable shapes the first build could take, with the trade-offs of each, rather than narrowing to a single recommendation.
---
