---
name: estimate-check
description: Makes an engineering estimate auditable for someone who can't audit it themselves. Use this skill PROACTIVELY when the user reports an estimate they've been given and wants to know if it's real — "our engineer says the refactor will take 6 weeks, is that right?", "the agency quoted 3 months for this", "my dev says we can't ship the feature without rebuilding X first", "is 2 weeks reasonable for [feature]?", "the team says it can't be done faster" — or when a PM or founder is deciding whether to approve an estimate, push back on one, or take a quote to a contractor. The core move is NOT re-estimating (Keel hasn't seen the code the estimator has) — it's arming the user with the three questions that make the estimate auditable, translating the estimate into money and opportunity cost, and identifying which follow-up answers would reveal padding, sandbagging, hidden complexity, or a genuinely honest number. Especially important for the non-technical PM segment, whose failure mode is deference — accepting numbers without the framing to question them. Does not trigger on the user asking Keel to estimate a feature from scratch (that's feature-decision's estimate section).
---
